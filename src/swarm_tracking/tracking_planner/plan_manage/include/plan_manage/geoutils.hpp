...
#ifndef MANAGE_CIRI_GEOUTILS_HPP
#define MANAGE_CIRI_GEOUTILS_HPP
...
inline bool findInterior(const Eigen::MatrixXd &hPoly,
  ...
  double minmaxsd = manage_sdlp::linprog(c, A, b, x);
  interior = x.head<3>();
...
inline void enumerateVs(const Eigen::MatrixXd &hPoly,
  ...
  manage_quickhull::manage_QuickHull<double> qh;
  double qhullEps = std::min(epsilon, manage_quickhull::defaultEps<double>());
