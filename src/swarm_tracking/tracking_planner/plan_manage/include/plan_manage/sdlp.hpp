#ifndef MANAGE_CIRI_SDLP_HPP
#define MANAGE_CIRI_SDLP_HPP

#include <Eigen/Eigen>
#include <cmath>
#include <random>

namespace manage_sdlp {
...
*/
{
  ...
  int i, status = manage_sdlp::AMBIGUOUS;
  ...
  status = manage_sdlp::linfracprog(halves, 0, m, n_vec, d_vec,
                             d, opt, work, next, prev, m);

  /* handle states for linprog whose definitions differ from linfracprog */
  if (status != manage_sdlp::INFEASIBLE) {
    if (xf(d) != 0.0 && status != manage_sdlp::UNBOUNDED) {
      x = xf.head(d) / xf(d);
      minimum = c.dot(x);
    }
