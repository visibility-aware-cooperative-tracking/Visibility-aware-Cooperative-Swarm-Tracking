...
// Three dimensional 7-order Trajectory
...
// One dimensional 3-order Trajectory
typedef Eigen::Matrix<double, 1, 4> CoefficientMat1D;
typedef Eigen::Matrix<double, 1, 3> VelCoefficientMat1D;
typedef Eigen::Matrix<double, 1, 2> AccCoefficientMat1D;
...
/* -- Minimally Implementated 1D piece -- */
class Piece1D {
 private:
  double duration;
  CoefficientMat1D coeffMat;

 public:
  Piece1D() = default;

  Piece1D(double dur, const CoefficientMat1D &cMat)
      : duration(dur), coeffMat(cMat) {}
  ...
  inline const CoefficientMat1D &getCoeffMat() const {
    return coeffMat;
  }
  ...
  inline double getJer(const double &t) const {
    double jer{0.0};
    double tn = 1.0;
    int l = 1;
    int m = 2;
...
/* -- Minimally Implementated 1D trajectory -- */
class Trajectory1D {
 private:
  typedef std::vector<Piece1D> Pieces;
  Pieces pieces;
 ...
 public:
  ...
  Trajectory1D(const std::vector<double> &durs,
               const std::vector<CoefficientMat1D> &cMats) {
    int N = std::min(durs.size(), cMats.size());
    pieces.reserve(N);
    for (int i = 0; i < N; i++) {
      pieces.emplace_back(durs[i], cMats[i]);
  ...
  inline Eigen::VectorXd getPositions() const {
    int N = getPieceNum();
    Eigen::VectorXd positions(N + 1);
    for (int i = 0; i < N; i++) {
      positions(i) = pieces[i].getCoeffMat()(7);
    }
    positions(N) = pieces[N - 1].getPos(pieces[N - 1].getDuration());
    return positions;
  }
  ...
  inline double getJer(double t) const {
    int pieceIdx = locatePieceIdx(t);
    return pieces[pieceIdx].getJer(t);
  }

  inline Piece1D getPiece(int i) const
  {
    return pieces[i];
  }
};