...
namespace display
{
    ...
    private:
                            ...
                            double a = 1)
        {
            ...
            case steelblue:
                ...
                break;
            case olivegreen:
                marker.color.r = 69/255.0;
                marker.color.g = 139/255.0;
                marker.color.b = 0.0;
                break;
            }
        }
    ...
    public:
        ...
        template <class PATH, class TOPIC>
        void visualize_balls_path(const PATH &path,
                             const TOPIC &topic,
                             const Color color = blue,
                             ...
                             const double a = 0.2)
        {
            ...
            setMarkerColor(marker, color, a);
            visualization_msgs::MarkerArray marker_array;
            marker_array.markers.reserve(path.size() + 1);
            marker.action = visualization_msgs::Marker::DELETEALL;
            marker_array.markers.push_back(marker);
            marker.action = visualization_msgs::Marker::ADD;
            for (const auto &node : path)
            {
                setMarkerPose(marker, node[0], node[1], node[2]);
                auto d = 2 * radius;
                setMarkerScale(marker, d, d, d);
                marker_array.markers.push_back(marker);
                marker.id++;
        ...
        void visualize_arrows(const ARROWS &arrows, const TOPIC &topic, const Color &color)
        {
            ...
            setMarkerScale(arrow_msg, 0.05, 0.15, 0.18);//0.4,0.7,0 //0.1, 0.1 0 //0.01, 0.02
            setMarkerColor(arrow_msg, color, 1.0);
        ...
        void visualize_traj(const TRAJ &traj, const TOPIC &topic)
        {
            ...
            wayPts.push_back(traj.getPos(duration));
            visualize_path(path, topic);
            visualize_pointcloud(wayPts, std::string(topic) + "_wayPts");
        }

        template <class TRAJ, class TOPIC>
        void visualize_trajcps(const TRAJ &traj, const TOPIC &topic, const double& scale = 0.1, const Color color = red)
        {
            auto got = publisher_map_.find(topic);
            if (got == publisher_map_.end())
            {
                ros::Publisher pub =
                    nh_.advertise<visualization_msgs::Marker>(topic, 10);
                publisher_map_[topic] = pub;
            }
            ...
            for (const auto &piece : traj)
            {   
                step = piece.getDuration() / K;
                dt = step;
                for (int j = 0; j < K; ++j)
                {
                    cps.push_back(piece.getPos(dt));
                    dt += step;
                }
            }
            ...
            sphere.id = 0;
            line_strip.id = 0 + 1000;

            sphere.pose.orientation.w = line_strip.pose.orientation.w = 1.0;
            setMarkerColor(sphere, color, 0.7);
            setMarkerColor(line_strip, color, 0.7);
            sphere.scale.x = scale;
            sphere.scale.y = scale;
            sphere.scale.z = scale;
            line_strip.scale.x = scale * 1.1 / 3.0;
            ...
            for (int i = 0; i < int(cps.size()); i++)
            {
                ...
                if(i%2==0 || i==int(cps.size())-1) sphere.points.push_back(pt);
            }
        ...
        void visualize_fan_shape_meshes(const std::vector<Eigen::Vector3d>& v0,
            ...
            setMarkerColor(marker, green, 0.12);
            int M = v0.size();
            // int M = 1;
            for (int i = 0; i < M; ++i) {
        ...
        // TRIANGLE_FACET: std::vector<TRIANGLE_FACET> 
        // Each facet contains Vector3d vertices a_,b_,c_
        template <class TRIANGLE_FACET, class TOPIC>
        void visualize_mesh_and_edge(const TRIANGLE_FACET &facet_list, const TOPIC &topic,
                                     const Color color = steelblue, const double alpha = 0.4,
                                     const Color color_edge = black, const double alpha_edge = 0.01)
        {   
            auto got = publisher_map_.find(topic);
            if (got == publisher_map_.end())
            {
            ...
            mesh_msg.lifetime = ros::Duration();
            ...
            setMarkerPose(mesh_msg, 0, 0, 0);
            setMarkerColor(mesh_msg, color, alpha);
            setMarkerScale(mesh_msg, 1, 1, 1);
            std::vector<std::pair<Eigen::Vector3d, Eigen::Vector3d>> edges;
            for (size_t i = 0; i < facet_list.size(); i++) 
            {   
                edges.emplace_back(facet_list[i].a_, facet_list[i].b_);
                edges.emplace_back(facet_list[i].b_,facet_list[i].c_);
                edges.emplace_back(facet_list[i].a_,facet_list[i].c_);

                geometry_msgs::Point a;
                a.x = facet_list[i].a_(0);
                a.y = facet_list[i].a_(1);
                a.z = facet_list[i].a_(2);
                mesh_msg.points.push_back(a);
                geometry_msgs::Point b;
                b.x = facet_list[i].b_(0);
                b.y = facet_list[i].b_(1);
                b.z = facet_list[i].b_(2);
                mesh_msg.points.push_back(b);
                geometry_msgs::Point c;
                c.x = facet_list[i].c_(0);
                c.y = facet_list[i].c_(1);
                c.z = facet_list[i].c_(2);
                mesh_msg.points.push_back(c);
            }
            publisher_map_[topic].publish(mesh_msg);
            visualize_pairline(edges, std::string(topic) + "_edge",  color_edge, alpha_edge);
        }
