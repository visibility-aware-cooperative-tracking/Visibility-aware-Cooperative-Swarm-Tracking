...
// one dimension min_acc minco 
class MINCO_S2{
  public:
    int N;
    
    Eigen::MatrixXd headPV;
    Eigen::MatrixXd tailPV;
    ...
    inline void reset(const int &pieceNum) {
      ...
      T3.resize(N);
      A.create(4 * N, 4, 4);
      b.resize(4 * N, 1);//here ONE

      gdC.resize(4 * N, 1);//here ONE
      gdP.resize(1, N - 1);//here ONE
      gdT.resize(N);
      return;
    }

    inline void generate(const Eigen::MatrixXd &headState, //1x2
                         const Eigen::MatrixXd &tailState, //1x2
    ...
    inline void calGrads_PT() {
      ...
      gdTail = gdC.bottomRows(2).transpose();
      // addPropCtoT
      Eigen::MatrixXd B1(4, 1), B2(2, 1);//here ONE, //here ONE (row num)
      ...
      for(int i = 0; i < N - 1; i++) {
        ...
        gdT(i) += B1.cwiseProduct(gdC.block<4, 1>(4 * i + 2, 0)).sum();//here one
      }
      ...
      gdT(N - 1) += B2.cwiseProduct(gdC.block<2, 1>(4 * N - 2, 0)).sum();//here one
    ...
    inline Trajectory1D getTraj(void) const {
      Trajectory1D traj;
      traj.reserve(N);
      for (int i = 0; i < N; i++) {
        traj.emplace_back(T1(i), b.block<4, 1>(4 * i, 0).transpose().rowwise().reverse());
      }
      return traj;
    }
};
...
class MINCO_S4 {
 public:
  ...
  inline void generate(const Eigen::MatrixXd &headState,
  ...
  inline Eigen::MatrixXd getConstraintPoints(const int K)
  {
    Eigen::MatrixXd pts(3, N * K + 1);
    Eigen::Vector3d p; //constraintPoints include pos and yaw
