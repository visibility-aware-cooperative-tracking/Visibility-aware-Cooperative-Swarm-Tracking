...
ros::Publisher Vis_ID_publisher;
...
void VisualizeDroneID(const std_msgs::Header &header, const Eigen::Vector3d &posi) {
    visualization_msgs::Marker vis_drone_id;
    vis_drone_id.header = header;
    ...
    vis_drone_id.color.a = 1.0; 
    vis_drone_id.color.r = 0.0;
    vis_drone_id.color.g = 0.0;
    vis_drone_id.color.b = 0.0;
    vis_drone_id.scale.x = 0.2;
    vis_drone_id.scale.y = 0.2;
    vis_drone_id.scale.z = 0.2;
    ...
    vis_drone_id.pose.orientation.z = 0.0;
    vis_drone_id.pose.orientation.w = 1.0;

    vis_drone_id.text = "UAV" + to_string(drone_id);
    Vis_ID_publisher.publish(vis_drone_id);
}
...
void odom_callback(const nav_msgs::Odometry::ConstPtr &msg) {
    ...
    Eigen::Vector3d posi(pose(0), pose(1), pose(2));
    VisualizeDroneID(poseROS.header, posi);
    ...
    // Path
    static ros::Time first_odom_time = msg->header.stamp;
    static ros::Time prevt = msg->header.stamp;
    if ((msg->header.stamp - prevt).toSec() > 0.1 && (msg->header.stamp - first_odom_time).toSec() > 2.0) {
        prevt = msg->header.stamp;
        geometry_msgs::PoseStamped pose_lidar;
        pathROS.header = poseROS.header;
        pose_lidar = poseROS;
        pose_lidar.pose.position.x -= init_x;
...
void Syncallback(const nav_msgs::Odometry::ConstPtr &msg_me, const nav_msgs::Odometry::ConstPtr &msg_other,
                 const sensor_msgs::PointCloud2::ConstPtr &msg_pcl) {
    geometry_msgs::PoseStamped relative_pose;
    relative_pose.header = msg_me->header;

    Eigen::Matrix3d relative_rotation, rot_me, rot_other;
    Eigen::Vector3d relative_posistion, position_me, position_other;
    rot_me = Eigen::Quaterniond(msg_me->pose.pose.orientation.w, msg_me->pose.pose.orientation.x,
                                msg_me->pose.pose.orientation.y, msg_me->pose.pose.orientation.z).matrix();
    rot_other = Eigen::Quaterniond(msg_other->pose.pose.orientation.w, msg_other->pose.pose.orientation.x,
                                   msg_other->pose.pose.orientation.y, msg_other->pose.pose.orientation.z).matrix();

    position_me = Eigen::Vector3d(msg_me->pose.pose.position.x, msg_me->pose.pose.position.y,
                                  msg_me->pose.pose.position.z);
    position_other = Eigen::Vector3d(msg_other->pose.pose.position.x, msg_other->pose.pose.position.y,
                                     msg_other->pose.pose.position.z);
    //Compute relative pose
    relative_rotation = rot_me.transpose() * rot_other;
    relative_posistion = rot_me.transpose() * (position_other - position_me);

    relative_pose.pose.position.x = relative_posistion(0);
    relative_pose.pose.position.y = relative_posistion(1);
    relative_pose.pose.position.z = relative_posistion(2);

    Eigen::Quaterniond relative_quat(relative_rotation);
    relative_pose.pose.orientation.w = relative_quat.w();
    relative_pose.pose.orientation.x = relative_quat.x();
    relative_pose.pose.orientation.y = relative_quat.y();
    relative_pose.pose.orientation.z = relative_quat.z();
    pub_relative_pose.publish(relative_pose);

    sensor_msgs::PointCloud2 pcl_sync;
    pcl_sync = *msg_pcl;
    pub_sync_lidar.publish(pcl_sync);
}
...
int main(int argc, char **argv) {
    ...
    pub_relative_pose = n.advertise<geometry_msgs::PoseStamped>(relative_pose_topic, 1000);
    pub_sync_lidar = n.advertise<sensor_msgs::PointCloud2>(quad_name + "/sensor_cloud_sync", 1000);

    //message filter
    message_filters::Subscriber<nav_msgs::Odometry> sub_quadstate_me(n, "odom", 1000); 
    message_filters::Subscriber<nav_msgs::Odometry> sub_quadstate_other(n, sub_quadpose_topic, 1000); 
    message_filters::Subscriber<sensor_msgs::PointCloud2> sub_pointcloud(n, sub_pointcloud_topic, 1000); 
    ...
    pathPub = n.advertise<nav_msgs::Path>( "path", 1000, true);
    velPub = n.advertise<visualization_msgs::Marker>( "velocity", 1000, true);
    covPub = n.advertise<visualization_msgs::Marker>( "covariance", 1000, true);
    covVelPub = n.advertise<visualization_msgs::Marker>( "covariance_velocity", 1000, true);
    trajPub = n.advertise<visualization_msgs::Marker>( "trajectory", 1000, true);
    sensorPub = n.advertise<visualization_msgs::Marker>( "sensor", 1000, true);
    meshPub = n.advertise<visualization_msgs::Marker>("robot", 1000, true);
    heightPub = n.advertise<sensor_msgs::Range>(quad_name + "height", 1000, true);
    Vis_ID_publisher = n.advertise<visualization_msgs::Marker>("drone_id", 1000);
